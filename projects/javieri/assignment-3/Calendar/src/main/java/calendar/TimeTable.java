     ...
     **/
	  public LinkedList<CalDay> getApptRange(LinkedList<Appt> appts,GregorianCalendar firstDay, GregorianCalendar lastDay)throws DateOutOfRangeException{
		...
		for (int i = 0; i < appts.size(); i++) {
			...
			// the correct CalDay
			int daysDifference = 0; //[[ERROR]] Different error than assignment 1&2, should be =0;
	   ...
	   /**
	    ...
	    private static LinkedList<GregorianCalendar> getApptOccurences(Appt appt, 
	        ...
	        if (!firstDay.before(lastDay)) {  //[[REDUNDANT, UNTESTABLE due to earlier exception thrown]]
	            return result;
	        }
	    ...
	    public LinkedList<Appt> deleteAppt(LinkedList<Appt> appts,Appt appt) {
	        ...
	        for(int i=1;i<appts.size()-1;i++){   //[[ERROR: I didn't add this one. i should = 0??]]
	    ...
	    public LinkedList<Appt> permute(LinkedList<Appt> appts, int[] pv){
	    	...
	    	*/
	    	...
	    	int nexti =  0;
	    	for(int i = 0;i<pv.length;i++){		//[[ERROR: I didn't add this. Whole function seems to be wrong
	    	    int newi = pv[nexti];
	    	    newi = pv[nexti];		//[[Redunant]]
	    	   Collections.swap(apptsUpdatedList,newi,newi); //[[Wrong, just returns the same list as before]]
	    	   nexti = newi; //[[i never updates]]
	    	} 
